			...
			if (meshComppnent != nullptr) {
				...
				for (auto shape_i = 0; shape_i < mesh->shapeCount(); shape_i++)
				{
					...
					for (auto i = 0; i < shape->vertecesSize*3; i++)
					{
						...
						if (NewPos.y < rc_bmin[0]) rc_bmin[0] = NewPos.y;
						if (NewPos.z < rc_bmin[0]) rc_bmin[0] = NewPos.z;

						if (NewPos.x > rc_bmax[0]) rc_bmax[0] = NewPos.x;
						if (NewPos.y > rc_bmax[0]) rc_bmax[0] = NewPos.y;
						if (NewPos.z > rc_bmax[0]) rc_bmax[0] = NewPos.z;
					}
