...
void Attachable::OnStart() {
	...
	m_mesh->CreateMesh(&form.verteces, &form.indexes);

	form = Forms4::SphereLined(boundRadius, 8, 8, { 0, 0.6, 0, 1 });
	m_bound->CreateMesh(&form.verteces, &form.indexes);
	m_bound->mesh()->topology = form.topology;

	m_mesh->SetVisibility(showCenter);
	m_bound->SetVisibility(false);
}
...
void Attachable::OnUpdate() {
	...
	if (!m_canAttach) {
		if ((boundRadius / player->radius) <= 0.8f) {
			m_canAttach = true;
			m_bound->SetVisibility(true && showBound);
		}
	}
